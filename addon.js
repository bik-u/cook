...
async function decrypt(url) {
    ...
    return JSON.parse(crypto_js.AES.decrypt(final_url, key).toString(crypto_js.enc.Utf8))
}	
...
builder.defineStreamHandler(async({type, id}) => {
	...
	for (let i = 0; i < shows.length; i++) {
		...
		const response = await axios.get(url=`${ani_url}ajax/v2/episode/servers?episodeId=${show.episode.id}}`)
		let subtitles = null
		...
		if (response.data.status) {
			...
			for (let i = 0; i < matches.length; i++) {
				try {
					...
					const response2 = await axios.get(`${ani_url}ajax/v2/episode/sources?id=${srv_id}`)
